    ...
    try:
        ...
        with open(file_path, "rb") as media_file:
            files = {
                ...
                'file': (f"{video_title}.mp3", media_file, 'audio/mpeg')
            }
            drive_headers = {"Authorization": f"Bearer {req.access_token}"}
            upload_res = requests.post(upload_url, headers=drive_headers, files=files)
